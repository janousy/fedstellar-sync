...
for aggregator in aggregation_list:
    vals = mnist_loss.filter(regex=(f'{aggregator}_ERROR')).dropna()
...
fmnist_loss_sem = fmnist_b[fmnist_b.columns.drop(list(fmnist_b.filter(regex='__MIN')))]
...
cifar10_loss_sem = cifar10_b[cifar10_b.columns.drop(list(cifar10_b.filter(regex='__MAX')))]
cifar10_loss_sem = cifar10_loss_sem[cifar10_loss_sem.columns.drop(list(cifar10_loss_sem.filter(regex='__MIN')))]

tmp = pd.DataFrame(index=range(0,N_ROUNDS), columns=aggregation_list)
for aggregator in aggregation_list:
    vals = cifar10_loss_sem.filter(regex=(f'{aggregator}_ERROR')).dropna()
    vals = vals.reset_index().iloc[:, 1]
    assert len(vals) == N_ROUNDS
    tmp[aggregator] = vals
tmp = tmp.reindex(sorted(tmp.columns), axis=1)
...
file = "figures/baseline_overview_loss_sem.png"
