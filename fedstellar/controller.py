    ...
    def run_webserver(self):
        if sys.platform == "linux" and self.cloud:
            ...
            with open(f'{self.log_dir}/server.log', 'w', encoding='utf-8') as log_file:
                subprocess.Popen(["gunicorn", "--reload", "--workers", "10", "--threads", "2", "--bind", f"unix:/tmp/fedstellar.sock", "--access-logfile", f"{self.log_dir}/server.log", "app:app"], cwd=webserver_path, env=controller_env, stdout=log_file, stderr=log_file, encoding='utf-8')
