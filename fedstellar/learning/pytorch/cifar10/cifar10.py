...
#######################################
#  FederatedDataModule for CIFAR10  #
#######################################
...
class CIFAR10DATASET():
    ...
    def __init__(self, iid=True):
        ...
        # Singletons of CIFAR10 train and test datasets
        if not os.path.exists(f"{sys.path[0]}/data"):
            os.makedirs(f"{sys.path[0]}/data", exist_ok=True)
        
        self.trainset = CIFAR10(
                f"{sys.path[0]}/data", train=True, download=True, transform=transform
                )
        self.testset = CIFAR10(
                f"{sys.path[0]}/data", train=False, download=True, transform=transform
                )
